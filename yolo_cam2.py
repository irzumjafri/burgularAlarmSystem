...
cam_show = True
string ="live_cam"
video_out = cv2.VideoWriter(string+"3.avi",cv2.VideoWriter_fourcc('M','P','4','2'), 10, (640,480))
...
if args.mode == "online":
    results= open("results3.txt","w+")
    video_capture_0 = cv2.VideoCapture("testing_video_2.avi")
    ...
    while True:
        ...
        if keyboard.is_pressed('d'):
            ...
            if cam_show == False:
                cv2.destroyWindow("Live Cam 3")

        if (ret0) and (cam_show):
            cv2.imshow("Live Cam 3", image)
